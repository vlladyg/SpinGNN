"""Allegro Neural Network Modules with Spin Extensions.

This module contains the core Allegro architecture and its spin-extended variant.

=== ORIGINAL ALLEGRO (Allegro_Module) ===

The standard Allegro architecture from the paper "Learning Local Equivariant
Representations for Scalable Molecular Dynamics" (Musaelian et al., 2023).

Key features:
- Strictly local message passing with learned pair interactions
- Edge-based architecture (no explicit node features)
- Latent MLPs process scalar invariants
- Tensor products build equivariant features
- Per-layer cutoffs for computational efficiency

=== SPIN-EXTENDED ALLEGRO (Allegro_Module_SEGNN) ===

Extension of Allegro for magnetic systems. The key differences from standard
Allegro are marked with "(SPIN PART)" comments in the code.

Changes from original Allegro:
1. Additional required_irreps_in: NODE_SPIN_LENGTH, EDGE_SPIN_DISTANCE_EMBEDDING
2. Extended first-layer latent input includes:
   - Spin lengths of center and neighbor atoms
   - Spin distance embedding (radial basis of S_i · S_j)
   - Latent features from previous Allegro layer (for stacked architectures)

This allows the SEGNN to learn spin-dependent interactions while maintaining
equivariance under O(3) transformations.

=== USAGE IN SPINGNN++ ===

In SpinGNN++, the Allegro_Module_SEGNN receives latent features from the
preceding Allegro_Module_MSENN (position-only branch). This enables a
two-stage architecture:
1. MSENN: learns position-dependent exchange parameters
2. SEGNN: refines predictions using both position and spin information
"""
...
class Allegro_Module_SEGNN(GraphModuleMixin, torch.nn.Module):
    """Spin-Extended Allegro Module (SEGNN variant).
    
    This module extends the standard Allegro architecture to include spin
    degrees of freedom in the latent space. It is designed to be used after
    a position-only Allegro layer (like MSENN) in the SpinGNN++ architecture.
    
    === KEY DIFFERENCES FROM STANDARD ALLEGRO ===
    
    1. First-layer latent input includes spin invariants:
       - 2 × spin lengths (center and neighbor)
       - Spin distance embedding (radial basis of S_i · S_j)
       - Previous layer's edge latent features
    
    2. Requires additional input irreps:
       - NODE_SPIN_LENGTH: scalar spin magnitude per atom
       - EDGE_SPIN_DISTANCE_EMBEDDING: radial basis encoding
       - latent_out_field: latent features from previous layer
    
    The output is edge-wise latent features that encode both geometric
    and magnetic information about the local environment.
    """
    # saved params
    num_layers: int
    field: str
...
class Allegro_Module(GraphModuleMixin, torch.nn.Module):
    """Standard Allegro Module (position-only, no spin).
    
    This is the original Allegro architecture that processes only positional
    information. In SpinGNN++, this serves as the baseline for comparison
    and as the position-only branch.
    
    === ARCHITECTURE OVERVIEW ===
    
    The Allegro module uses a series of tensor products to build up local
    equivariant representations. At each layer:
    
    1. Latent MLP: processes scalar invariants to produce weights
    2. Environment embedding: weighted sum of neighbor features
    3. Tensor product: combines edge features with environment
    4. Linear projection: reduces to target irreps
    5. Scalar extraction: invariants for next layer's latent MLP
    
    The latent space uses a ResNet-style update to maintain gradient flow.
    
    === INPUTS ===
    
    Required fields:
    - EDGE_ATTRS_KEY: spherical harmonics of edge vectors
    - EDGE_EMBEDDING_KEY: radial basis encoding of distances
    - NODE_ATTRS_KEY: one-hot atom type encoding
    
    === OUTPUTS ===
    
    - latent_out_field (default: EDGE_FEATURES): scalar edge features
      that can be used for energy prediction or passed to subsequent layers
    """
    # saved params
    num_layers: int
    field: str
