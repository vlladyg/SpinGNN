...
class EdgewiseFSum(GraphModuleMixin, torch.nn.Module):
    """Sum edgewise energies.
    ...
    def __init__(
        ...
        self._init_irreps(
            irreps_in=irreps_in,
            my_irreps_in={_keys.EDGE_FEATURES_F: irreps_in[_keys.EDGE_FEATURES_F]},
            irreps_out={_keys.NODE_FEATURES_F: irreps_in[_keys.EDGE_FEATURES_F]},
        )
        ...
        self.per_edge_species_scale = per_edge_species_scale
        if self.per_edge_species_scale:
            self.per_edge_scales_f = torch.nn.Parameter(torch.ones(num_types, num_types))
        else:
            self.register_buffer("per_edge_scales_f", torch.Tensor())

    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        edge_center = data[AtomicDataDict.EDGE_INDEX_KEY][0]
        edge_neighbor = data[AtomicDataDict.EDGE_INDEX_KEY][1]

        edge_features_f = data[_keys.EDGE_FEATURES_F]
        species = data[AtomicDataDict.ATOM_TYPE_KEY].squeeze(-1)
        center_species = species[edge_center]
        neighbor_species = species[edge_neighbor]

        if self.per_edge_species_scale:
            edge_features_f = edge_features_f * self.per_edge_scales_f[
                center_species, neighbor_species
            ].unsqueeze(-1)

        node_features_f = scatter(edge_features_f, edge_center, dim=0, dim_size=len(species))
        factor: Optional[float] = self._factor  # torchscript hack for typing
        if factor is not None:
            node_features_f = node_features_f * factor

        data[_keys.NODE_FEATURES_F] = node_features_f
